---
name: review
description: Orca review stage — drives the independent cross-model Codex review for one work item through the codex MCP tool, writes the findings artifact verbatim, and returns the finding counts. Spawned by the orca:run skill; not for standalone use.
