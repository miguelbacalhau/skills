...
case "$verb" in
  commit-verify|merge-finalize|worktree-item|secrets|config|preflight|triage|review)
  ...
  self-test)
    ...
    ;;
  *)
    fail UNKNOWN_VERB "unknown verb '${verb}' — usage: orca.sh <verb> [args...]; verbs: commit-verify, merge-finalize, worktree-item, secrets, config, preflight, triage, review, self-test"
    ;;
esac
