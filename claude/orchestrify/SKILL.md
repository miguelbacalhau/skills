...
- **Codex** is the independent, cross-model reviewer in the work loop — a different model family from the Claude implementer, so it does not share the implementer's blind spots. A missing or unauthenticated `codex` would fail every item at review time, deep into an autonomous run. Fix: install with `npm i -g @openai/codex`, or authenticate with `codex login` (or `codex login --with-api-key`). Installing or authenticating is the user's action — surface the command, do not do it autonomously.
...
Workflow({
  ...
  args: {
    ...
    reviewScript: "/abs/path/to/skills/orchestrify/scripts/codex-review.sh",
