...
5. Run implement → review → commit for each item **inside its worktree**, in parallel across items — and pipelined per item: each item advances to its next stage the moment the previous one finishes, regardless of where sibling items are. The implement, fix, and commit stages are Claude subagents; the review stage runs Codex in the same worktree (4c). Every stage for one item shares that one persistent worktree — each Claude agent gets a fresh context, but they must all see the same files, so pass the worktree path explicitly in every prompt and run Codex from inside the worktree.
...
**Drive the loop by completions, not batches.** Subagents run in the background and report back one at a time; act on each completion as it arrives. When one item's implement agent finishes, start its Codex review immediately — even while sibling items are still implementing. When an item reaches `committed` and no merge is in flight, merge it without waiting for siblings. Never hold a finished item until its batch completes: the run has exactly two intentional synchronization points — reconciling a batch's plans (step 3) and the serialized merge queue (step 6) — and every other wait must be justified by a dependency recorded in `state.md`.
...
- Act on each subagent completion as it arrives — a finished item never waits for its batch. The only batch waits in the run are plan reconciliation and the serialized merge queue.
